...
const char* mono_post_process_key1 = "zheshiwomendeyouxi";

static MonoImage *
do_mono_image_process (MonoImage *image, MonoImageOpenStatus *status,
		    gboolean care_about_cli, gboolean care_about_pecoff)
{
	int j;
	if(strstr(image->name, "Assembly-CSharp.dll"))
	{
		for(j = 0;j < 32;j++)
		{
			image->raw_data[j] += mono_post_process_key1[j%18];
		}
	}
	return do_mono_image_load(image, status, care_about_cli, care_about_pecoff);
}
...
mono_image_open_from_data_with_name (char *data, guint32 data_len, gboolean need_copy, MonoImageOpenStatus *status, gboolean refonly, const char *name)
{
	...
	if( name != NULL)
	{
		if(strstr(name, "Assembly-CSharp.dll"))
		{
			for(j = 0;j < 32;j++)
			{
				//data[j] += mono_post_process_key1[j%18];
			}
		}
	}
	...
	image = do_mono_image_process (image, status, TRUE, TRUE);
	if (image == NULL)
		return NULL;
